...
uniform bool     light_enable_state[8]; // booleans to indicate which of eight lights 
                                        // are enabled
...
attribute vec4   a_texcoord2;   // available if enable_tex[1] is true
attribute vec4   a_color;       // available if !enable_lighting or (enable_lighting && enable_color_material)
    ...
    if (att_factor > c_zero)
    {
        ...
        if (ndoth > c_zero)
        {
            computed_color += (pow(ndoth, material_state.specular_exponent) *
                    material_state.specular_color * light_state[i].specular_color);
        }
